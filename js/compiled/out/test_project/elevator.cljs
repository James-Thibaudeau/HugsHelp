...
(defn game-timer []
  (reagent/with-let [timer-fn (js/setInterval #(do
                                                 (js/console.log "timer")
                                                 (swap! game-state update :time dec)) 1000)]
                    [:div.timer
...
(defn elevator []
  (reagent/create-class
    {:component-did-mount
     ...
     (fn []
       (.removeEventListener js/document "keydown" move))
     :reagent-render
     (fn []
       [:div.container.is-fluid
        ...
        (if (:game-over? @game-state)
          [:div
           [:h2.title "GAME OVER -- " (when (= 0 (:failures @game-state)) "PERFECT!")]
